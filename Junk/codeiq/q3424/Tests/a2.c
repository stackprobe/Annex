...
static autoList_t *GetRs(uint rNum)
{
	...
	// cache
	if(refElement(RCache, rNum))
		return getList(RCache, rNum);
...
//			releaseDim(rs2, 1);
	...
	// cache
	putElement(RCache, rNum, (uint)dest);

	cout("%u -> %08x %u\n", rNum, dest, getCount(dest));
...
//	releaseDim(rs, 1); // キャッシュされるから開放しちゃだめ！
}
