...
//	static double *cache[STARV_DAY_MAX + 1][AZUKE_DAY_MAX + 1][100 + 1];
	...
	if(!cache)
		cache = (double *(*)[AZUKE_DAY_MAX + 1][100 + 1])memAlloc(sizeof(*cache) * (AZUKE_DAY_MAX + 1));

	if(!cache[starvDay][azukeDay][gohanPct])
	{
		double *pPct = (double *)memAlloc(sizeof(double));

LOGPOS();
		*pPct = Pet2_Main(starvDay, azukeDay, gohanPct);
LOGPOS();

		cache[starvDay][azukeDay][gohanPct] = pPct;
	}
